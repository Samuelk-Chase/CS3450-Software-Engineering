    ...
    try {
      ...
      if (aiMessage.includes("*Receive card reward*")) {
        const cardResponse = await axios.post("http://localhost:8080/v1/card", {
          name: "Special Card",  // Example name, will change when card endpoint updated
          description: "This is a special card generated by the story.",  // Example description, wiull change
              ...
              {chatHistory.map((entry, index) => (
                <div key={index} className={`chat-entry ${entry.sender}`}>
                  <span className="timestamp">[{entry.timestamp}]</span>
                   {/* very sloppy way to replace text. Will change, didn't want to deal with state issues for now */}
                  <span className="chat-text">{entry.text.replace("*Receive card reward*", "").replace("*Boss combat begins.*", "")}</span>
                </div>
              ))}
            </div>
